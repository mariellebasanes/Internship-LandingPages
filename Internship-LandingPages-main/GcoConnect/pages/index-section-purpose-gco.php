...
<section class="bg-body py-20 overflow-hidden position-relative" style="z-index: 0;">
  ...
  <img src="<?php echo htmlspecialchars($assetsBase ?? 'assets'); ?>/img/bg-assets/human-brain.png" class="position-absolute d-none d-lg-block" style="bottom: 10%; left: 2%; opacity: 0.4; pointer-events: none; width: 280px; transform: rotate(-15deg); z-index: 0;" alt="">
  <img src="<?php echo htmlspecialchars($assetsBase ?? 'assets'); ?>/img/bg-assets/flowers.png" class="position-absolute d-none d-lg-block" style="top: 15%; right: 5%; opacity: 0.4; pointer-events: none; width: 240px; transform: rotate(10deg); z-index: 0;" alt="">
  <div class="container position-relative" style="z-index: 1;">

    <div class="text-center mb-15">
      <span class="badge badge-light-primary fs-9 ls-2 text-uppercase fw-bold py-2 px-4 mb-4">Purpose of Appointment</span>
      <h2 class="fw-bolder fs-2x mb-4">Find the <span class="text-primary">Help You Need!</span></h2>
      <p class="text-gray-600 fs-5 mw-600px mx-auto">
        Select your purpose and get the <span class="text-primary fw-semibold">support you deserve</span>&mdash;quick, easy, and just for you!
      </p>
    </div>
    ...
    <div class="mw-900px mx-auto">
      ...
      <div class="purpose-tab-nav d-flex flex-nowrap gap-3 justify-content-center mb-8">
        ...
        <label for="purpose-tab-<?= $i ?>"
          ...
          <i class="ki-duotone <?= $cat['icon'] ?> fs-5 me-2">
            <?php for ($p = 1; $p <= $cat['paths']; $p++): ?><span class="path<?= $p ?>"></span><?php endfor; ?>
          </i>
          <?= htmlspecialchars($cat['tab'] ?? $cat['title']) ?>
          <span class="badge badge-light-primary ms-2 fs-9"><?= $cat['count'] ?></span>
        </label>
        <?php endforeach; ?>
      </div>
      ...
      <div class="purpose-tab-pane" data-pane="<?= $i ?>">
        <div class="card card-bordered overflow-hidden">
          ...
          <!-- Card header -->
          <div class="card-header bg-<?= $cat['color'] ?> py-6 border-0 d-flex align-items-center justify-content-center gap-4">
            ...
            <div>
              <h3 class="text-white fw-bolder fs-3 mb-1"><?= htmlspecialchars($cat['title']) ?></h3>
              <span class="badge badge-light text-gray-700 fs-9"><?= $cat['count'] ?> options</span>
            </div>
          </div>
                ...
                <div class="card card-bordered mb-3">
                  <div class="card-body py-4 px-5 d-flex align-items-start gap-3">
                    <div class="symbol symbol-35px flex-shrink-0">
                      <span class="symbol-label bg-<?= $cat['color'] ?> text-white fw-bold fs-8"><?= $num ?></span>
                    </div>
                    <div>
                      <div class="fw-semibold fs-7 mb-1"><?= htmlspecialchars($item['title']) ?></div>
                      <div class="text-gray-500 fs-8"><?= htmlspecialchars($item['desc']) ?></div>
                    </div>
                  </div>
                </div>
                <?php endforeach; ?>
            ...
            <div class="card card-bordered mb-3">
              <div class="card-body py-4 px-5 d-flex align-items-start gap-4">
                <div class="symbol symbol-40px flex-shrink-0">
                  <span class="symbol-label bg-<?= $cat['color'] ?> text-white fw-bolder fs-6"><?= $idx + 1 ?></span>
                </div>
                <div>
                  <div class="fw-semibold fs-5 mb-1"><?= htmlspecialchars($item['title']) ?></div>
                  <div class="text-gray-500 fs-7"><?= htmlspecialchars($item['desc']) ?></div>
                </div>
              </div>
            </div>
            <?php endforeach; ?>
